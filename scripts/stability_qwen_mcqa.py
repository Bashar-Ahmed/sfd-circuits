"""
Cross-Input Stability Analysis on Qwen2.5-0.5B MCQA.

Extends the stability analysis from InterpBench IOI and GPT2-IOI to a new
model+task combination: Qwen2.5-0.5B on the MIB MCQA (copycolors) task.

Key differences from previous analyses:
  - Model is larger: 24 layers, 14 heads, 896 hidden → more edges
  - MCQA task has only 110 train examples → we use 5 splits × 20 examples
  - No ground-truth circuit available → we focus on stability, not accuracy
  - Uses the same safe-flow pipeline as GPT2 and InterpBench

Metrics:
  1. Split-to-split Spearman ρ (5 splits × 20 examples)
  2. Top-k Jaccard overlap between splits
  3. Per-edge score CV
  4. Size convergence (5, 10, 20 examples)
...
def load_qwen():
    """Load Qwen2.5-0.5B via TransformerLens."""
...
def dataloader_split(model, seed, num_examples=20, batch_size=5):
    """Create a dataloader for a specific random subset of MCQA examples."""
    ds = HFEAPDataset(
        "mib-bench/copycolors_mcqa", model.tokenizer,
        split="train", task="mcqa",
...
def run_attribution_qwen(model, dataloader, method, ig_steps=5):
    """Run edge attribution on the Qwen model graph."""
    graph = Graph.from_model(model)
    metric = get_metric("logit_diff", "mcqa", model.tokenizer, model)
...
def compute_stability(all_scores, edge_names):
    ...
    # Most stable edges (top-100 consistently across splits)
...
def categorize_edge(edge_name):
    """Classify edge type from its name."""
    src, dst = edge_name.split("->", 1) if "->" in edge_name else ("?", "?")
    if "logits" in dst:
        cat = "→logits"
    elif "input" in src or "embed" in src.lower():
        cat = "embed→*"
    elif ".h" in src and ".h" in dst:
        cat = "attn→attn"
    elif "m" in src and "m" not in dst:
        cat = "MLP→attn"
    elif "m" not in src and "m" in dst:
        cat = "attn→MLP"
    elif "m" in src and "m" in dst:
        cat = "MLP→MLP"
    else:
        cat = "other"
    has_mlp = "m" in src or "m" in dst
    return cat, has_mlp
...
def main():
    t0 = time.time()
    print("=" * 72)
    print("Cross-Input Stability on Qwen2.5-0.5B MCQA")
    print("=" * 72)
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # Test 1: Split-to-split stability (5 splits × 20 examples)
    # ═══════════════════════════════════════════════════════════════════════
    print("\n── Test 1: Split-to-split stability (5 × 20 examples) ──")
    print("  (MCQA train set has only 110 examples; using smaller splits)")
    N_SPLITS = 5
    ...
    for split_idx in range(N_SPLITS):
        dl = dataloader_split(model, seed=split_idx * 13, num_examples=20)
        for src in ["EAP", "EAP-IG-inputs"]:
            ...
            # Print diagnostics from first split
    ...
    print(f"  Qwen MCQA graph: {len(edge_names)} edges")
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # Test 2: Size convergence (5, 10, 20 examples)
    # ═══════════════════════════════════════════════════════════════════════
    print("\n── Test 2: Size convergence (5 → 10 → 20) ──")
    SIZES = [5, 10, 20]
    ...
    for method in METHODS:
        src, var = method
        label = f"{src}+{var}"
        ref_scores = size_scores[method][20]
    ...
    print(f"\n{'='*72}")
    print("RESULTS: Split-to-Split Stability (Qwen2.5-0.5B MCQA, 5 × 20 examples)")
    print(f"{'='*72}")
    ...
    # ── Size convergence ──
    print(f"\n{'='*72}")
    print("RESULTS: Size Convergence (vs 20-example reference)")
    print(f"{'='*72}")

    header2 = (f"  {'Method':<24s} {'5→20 ρ':>10s} {'10→20 ρ':>10s} "
               f"{'5→20 top50':>13s} {'10→20 top50':>13s}")
    print(f"\n{header2}")
    print(f"  {'-'*70}")
    for method in METHODS:
        label = f"{method[0]}+{method[1]}"
        sc = size_conv[label]
        print(f"  {label:<22s} {sc['5']['spearman_with_20']:>9.4f} "
              f"{sc['10']['spearman_with_20']:>9.4f} "
              f"{sc['5']['topk_overlap']['50']:>13.3f} "
              f"{sc['10']['topk_overlap']['50']:>13.3f}")

    # Fastest converger
    best_conv = max(size_conv.items(),
                    key=lambda x: x[1]["5"]["spearman_with_20"])
    print(f"\n  Fastest converger (5→20): {best_conv[0]} "
          f"(ρ={best_conv[1]['5']['spearman_with_20']:.4f})")

    # ── Most stable edges ──
    print(f"\n{'='*72}")
    print("Most Consistently Top-100 Edges Across 5 Splits")
    print(f"(edges appearing in top-100 in all 5 splits)")
    print(f"{'='*72}")

    for method in METHODS:
        label = f"{method[0]}+{method[1]}"
        stable_edges = stability[label]["most_stable_top100"]
        all5 = [(e, f) for e, f in stable_edges if f == 5]
        in45 = [(e, f) for e, f in stable_edges if f >= 4]
        print(f"\n  {label}:")
        print(f"    In 5/5 splits ({len(all5)}): "
              f"{', '.join(e for e,f in all5[:8])}"
              f"{'...' if len(all5) > 8 else ''}")
        print(f"    In ≥4/5 splits ({len(in45)}): "
              f"{', '.join(e for e,f in in45[:5])}"
              f"{'...' if len(in45) > 5 else ''}")

    # ── Edge type distribution of stable edges ──
    print(f"\n{'='*72}")
    print("Edge-Type Distribution of Most Stable Top-100 Edges (≥4/5 splits)")
    print(f"{'='*72}")

    for method in METHODS:
        label = f"{method[0]}+{method[1]}"
        stable_edges = stability[label]["most_stable_top100"]
        in45 = [e for e, f in stable_edges if f >= 4]

        # Categorize edges
        direct_logits = sum(1 for e in in45 if "->logits" in e)
        embed_src = sum(1 for e in in45 if "input->" in e or "embed" in e.split("->")[0].lower())
        mlp_involved = sum(1 for e in in45 if "m" in e.split("->")[0] or "m" in e.split("->")[1])
        attn_to_attn = sum(1 for e in in45 if ".h" in e.split("->")[0] and ".h" in e.split("->")[1])

        print(f"  {label}: {len(in45)} edges (≥4/5 splits)")
    ...
    # ═══════════════════════════════════════════════════════════════════════
    print(f"\n{'='*72}")
    print("CROSS-MODEL COMPARISON: Qwen2.5-0.5B MCQA vs GPT2-IOI vs InterpBench IOI")
    print(f"{'='*72}")

    # Data from previous experiments
    prior_results = {
        # (model, task, n_edges): {method: (sp_rho, score_cv)}
    ...
    print(f"\n  {'':30s} {'Spearman ρ':>12s} {'Score CV':>10s} {'ρ vs raw':>10s} {'CV vs raw':>10s}")
    for (model_name, task, n_edges), methods in prior_results.items():
        print(f"  --- {model_name} {task} ({n_edges} edges) ---")
        for m_label, (sp, cv) in methods.items():
            print(f"  {m_label:<28s} {sp:>11.3f} {cv:>9.3f}")

    print(f"\n  --- Qwen2.5-0.5B MCQA ({len(edge_names)} edges) ---")
    for method in METHODS:
        label = f"{method[0]}+{method[1]}"
        st = stability[label]
        print(f"  {label:<28s} {st['spearman_mean']:>11.4f} {st['cv_per_edge_mean']:>9.3f}")

    # ── VERDICT ──
    print(f"\n{'='*72}")
    print("VERDICT: Qwen2.5-0.5B MCQA Cross-Input Stability")
    print(f"{'='*72}")
    ...
    # Check: does safe-flow σ outperform raw on this larger model?
    ...
    output = {
        "test": "cross_input_stability_qwen_mcqa",
        "model": "qwen2.5-0.5B",
        "task": "mcqa",
    ...
    with open(f"{OUT}/stability_qwen_mcqa.json", "w") as f:
        json.dump(output, f, indent=2, default=convert)
    print(f"\nSaved → {OUT}/stability_qwen_mcqa.json")
